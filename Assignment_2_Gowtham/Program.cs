    ...
    class Program
    {
        ...
        public static int RemoveDuplicates(int[] nums)
        {
            try
            {
                if (nums == null || nums.Length == 0)
                    return 0; // Returning the 0 if the array is empty or null

                int k = 1; // Starting with 1 as the first element is always unique
                ...
                return k; // Returning the count of unique elements
            }
            catch (Exception)
            {
                throw;
        ...
        public static IList<int> MoveZeroes(int[] nums)
        {
            try
            {
                if (nums == null || nums.Length <= 1)
                    return nums.ToList(); // Returning the list if it's null or has only one element

                int nonZeroIndex = 0; // Keeping track of the position for non-zero elements
                for (int i = 0; i < nums.Length; i++)
                {
                    if (nums[i] != 0)
                    {
                        nums[nonZeroIndex] = nums[i]; // Moving non-zero element to the current non-zero index
                        nonZeroIndex++; // Incrementing the non-zero index
                    }
                }
                for (int i = nonZeroIndex; i < nums.Length; i++) // Filling the remaining positions with zeroes from the last non-zero index
                {
                    nums[i] = 0; // Set remaining positions to zero
                }
                return nums.ToList(); // Converting the array to a list and return
            }
            catch (Exception)
            {
                throw;
        ...
        */
        ...
        public static IList<IList<int>> ThreeSum(int[] nums)
        {
            try
            {
                IList<IList<int>> result = new List<IList<int>>(); // Initializing the result list
                ...
                for (int i = 0; i < nums.Length - 2; i++)
                {
                    ...
                    while (left < right)
                    {
                        int sum = nums[i] + nums[left] + nums[right]; // Calculating the sum of the triplet

                        if (sum == 0)
                        {
                            // Founded a triplet with sum equal to zero
                            ...
                            right--; // Moving the right pointer to the left
                        }
                        else if (sum < 0)
                        {
                            left++; // Moving the left pointer to increase sum
                        }
                        else
                        {
                            right--; // Moving the right pointer to decrease sum
                        }
                    }
                }
                return result; // Returning the list of triplets
            }
            catch (Exception)
            {
                throw;
        ...
        */
        ...
        public static int FindMaxConsecutiveOnes(int[] nums)
        {
            try
            {
                int maxConsecutiveCount = 0; // Maximum of consecutive count of 1's
                ...
                foreach (int num in nums)
                {
                    ...
                    {
                        // Updating maximum count and reset current count when encountering 0
                        maxConsecutiveCount = Math.Max(maxConsecutiveCount, currentConsecutiveCount);
                        currentConsecutiveCount = 0;
                    }
                }

                // Updating maxConsecutiveCount for the last sequence of consecutive 1's
                maxConsecutiveCount = Math.Max(maxConsecutiveCount, currentConsecutiveCount);
        ...
        */
        ...
        public static int BinaryToDecimal(int binary)
        {
            try
            {
                int decimalValue = 0; // Initializing the decimal value to 0
                int baseValue = 1; // Initializing the base value to 1

                while (binary != 0)
                {
                    int lastDigit = binary % 10; // Extracting the last digit of the binary number
                    binary /= 10; // Removing the last digit from the binary number

                    decimalValue += lastDigit * baseValue; // Update the decimal value
                    baseValue *= 2; // Updating the base value power of 2
                }
        ...
        */
        ...
        public static int MaximumGap(int[] nums)
        {
            try
            {
                ...
                // Finding the below  minimum and maximum elements in the array
                ...
                // Calculating the minimum possible gap
                int minGap = (int)Math.Ceiling((double)(maxElement - minElement) / (nums.Length - 1));

                // Creating the buckets to store elements
                ...
                // Distributing the numbers into buckets
                foreach (int num in nums)
                {
                ...
                // Finding the maximum gap between the successive non-empty buckets
                int maxGap = 0;
        ...
        */
        ...
        public static int LargestPerimeter(int[] nums)
        {
            try
            {
                Array.Sort(nums); // Sorting the array in non-decreasing order
                for (int i = nums.Length - 1; i >= 2; i--) // Iterate from right to left
                {
                    int sideA = nums[i - 2];
                    int sideB = nums[i - 1];
                    int sideC = nums[i];
                    if (sideA + sideB > sideC) // Checking if a valid triangle can be formed
                    {
                        return sideA + sideB + sideC; // Returning the perimeter of the triangle
                    }
                }
                return 0;
            }
        ...
        */
        ...
        public static string RemoveOccurrences(string s, string part)
        {
            try
            {
                ...
                {
                    // Removing the leftmost occurrence of the substring part
